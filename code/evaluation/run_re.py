...
class MnliProcessor0(DataProcessor):
...
class MnliProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, "multinli_1.0_train.jsonl")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, "unbiased_eval.jsonl")), "dev")
...
class MnliProcessorOnlyHyp(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, "T4_train_causes.jsonl")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, "T4_causes.jsonl")), "dev")

    def get_labels(self):
        """See base class."""
        return ["None","satisfies","controls","increases","decreases"]
...
class FeverProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, "snli_1.0_train.jsonl")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            #self._read_jsonlines(os.path.join(data_dir, "train.jsonl")), "train")
            self._read_jsonlines(os.path.join(data_dir, "snli_1.0_dev.jsonl")), "dev")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if False:
                #random label
                label = target_labels[random.randint(0, num_labels-1)]
...
class FeverProcessorOnlyClaim(DataProcessor):
    """Processor for the FEVER data set using only the claims"""
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, label=label, weight=0.0))
        return examples
...
def main():
    ...
    processors = {
        "cola": ColaProcessor,
        "mrpc": MrpcProcessor,
        "fever_only_claim": FeverProcessorOnlyClaim,
        "fever": FeverProcessor,
        "mnli": MnliProcessor,
        "re_task": MnliProcessorOnlyHyp,
    }
    ...
    if args.do_train:
        ...
        model.train()
        #num_batch_per_epoch = len(train_examples)//args.train_batch_size
        #t_total = int(num_batch_per_epoch \
        #        // args.gradient_accumulation_steps*args.num_train_epochs)
        #scheduler = get_linear_schedule_with_warmup(optimizer,
        #        num_warmup_steps = 1000,#int(0.06*t_total),
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss, _ = model(input_ids, segment_ids, input_mask, label_ids, weights)
                if n_gpu > 1:
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if args.output_preds:
                ...
                exp_probs = []
                for prob in probs:
                    exp_probs.append([math.exp(s) for s in prob])
                s_e_probs = [[sum(s) for _ in range(len(label_list))]\
                        for s in exp_probs]
                probs = []
                for s,s_e_prob in zip(exp_probs,s_e_probs):
                    prob = []
                    for x,y in zip(s,s_e_prob):
                        prob.append(x/y)
                    probs.append(prob)
                for prob in probs:
                    dict = {}
